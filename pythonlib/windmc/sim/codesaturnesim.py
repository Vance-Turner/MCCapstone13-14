...
class CodeSaturneSim():
    ...
    def codeSaturneSim(self,name,shroudRawPoints,headLoss=0.25,meshType=meshTypes_BUILD,meshCode='',codeSatType=codeSatActDisk):
        ...
        if meshType==meshTypes_COPY:
            ...
            # Get the reachable meshing services
            reachable = utilities.getActiveMeshServices()
            # Now pick a random node to send job to:
            print "Got reachable nodes:",reachable
            node = random.randint(0,len(reachable)-1)
            print "Going to send job to:",node,reachable[node]             
            ...
            jobResponse = requests.post(reachable[node]+'/meshrequest/',data=jobData)
