...
class MeshingService(Thread):
    ...
    def run(self):
        ...
        while(self.alive):
            if len(self.meshTaskQueue)>0:
                lst = self.meshTaskQueue.pop()
                lst[0].run()
    ...
    def submitJob(self,task):
        id = uuid.uuid1()
        self.jobMap[id]=False
        self.meshTaskQueue.append([task,id])
        return id

    def isJobCompleted(self,id):
        if self.jobMap.has_key(id):
...
class CodeSatMeshGenTask(Thread):
    """
