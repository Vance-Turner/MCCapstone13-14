...
p2_top = MakeVertex(0,duct_height,extru)
...
# bottom
sub_faces = GetShapesOnPlane(v_duct, ShapeType["FACE"], dz, GEOM.ST_ON)
...
# top
sub_faces = GetShapesOnPlaneWithLocation(v_duct, ShapeType["FACE"], dz, p2_top, GEOM.ST_ON)
...
g_let2 = GetEdgesByLength(v_duct, duct_height+(2*duct_y_addition), duct_height+(2*duct_y_addition), 1, 1)
g_let = GetEdgesByLength(v_duct,duct_height,duct_height,1,1)
addToStudyInFather(v_duct,g_let2,"let2")
...
# so, we have to divide the outlet now into pieces! Do just like inlet.
algo1d_let_out = mesh_d.Segment(g_let2)
seg = algo1d_let_out.NumberOfSegments(50)
seg.SetDistrType(3)
seg.SetConversionMode(1)
seg.SetExpressionFunction('(t-0.5)^2+0.1')
